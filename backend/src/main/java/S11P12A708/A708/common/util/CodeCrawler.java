...
public class CodeCrawler {
    ...
    public static String bojCrawl(String nickname,String repo,int pbNum)  {
        ...
        url = url.replace("name",nickname)
                ...
                .replace("lang",temp.getName()+".java");
        return getJavaContent(url);

    }

    public static String handleEscapes(String input) {
    ...
    public static String getJavaContent(String url) {
    ...
    public static String getPythonContent(String url) {
        Document doc = null; // URL을 실제 URL로 대체하세요.
        try {
            String newurl = url.replace(".java",".py");

            doc = Jsoup.connect(newurl.replace("%20","%E2%80%85").replace(" ","%E2%80%85").replace("-","%EF%BC%8D")).get();
            // <script> 태그의 data-target 속성 값이 'react-app.embeddedData'인 태그를 찾기
            Elements scriptTags = doc.select("script[data-target=react-app.embeddedData]");
    ...
    public static String getCppContent(String url) {
        Document doc = null; // URL을 실제 URL로 대체하세요.
        try {
            String newurl = url.replace(".java",".cc");
            doc = Jsoup.connect(newurl.replace("%20","%E2%80%85").replace(" ","%E2%80%85").replace("-","%EF%BC%8D")).get();
            // <script> 태그의 data-target 속성 값이 'react-app.embeddedData'인 태그를 찾기
            Elements scriptTags = doc.select("script[data-target=react-app.embeddedData]");
